...
def count_0_in_seq(input_seq, count_type):
    ...
    if count_type == 'task2':
        ...
        # assign parameters
        assign.assign_weight_count_task_2(cell, in_dim=10, out_dim=2)
        # initial the first state
        prev_state = [0., 0.]
        # read input sequence one by one to count the digits
        for idx, d in enumerate(input_seq):
            prev_state = cell.run_step([d], prev_state=prev_state)
            #print("{}-th, data: {}; output: {}".format(idx, d, prev_state))
    ...
    if count_type == 'task3':
        ...
        cell = LSTMcell(in_dim=10, out_dim=2)
        # assign parameters
        assign.assign_weight_count_task_3(cell, in_dim=10, out_dim=2)
        # initial the first state
        prev_state = [0., 0.]

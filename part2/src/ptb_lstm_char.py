...
def evaluate_model(model_path, test_data, time_step = 240):
    model = load_model(model_path, custom_objects = {'perplexity': perplexity})
    ...
    result = model.evaluate(valid_x, batch_size = 1)
    print(result)

def fit_model(train_data, valid_data, vocab_size, train_portion, time_step = 240, batch_size = 30):
    ...
    model = Sequential()
    model.add(Embedding(vocab_size, 6, batch_input_shape = (batch_size, train_x.shape[1])))
    ...
    csv_logger = CSVLogger("ptb_char.csv", append=True, separator=',')
    checkpoint = ModelCheckpoint('ptb_char_best.h5', monitor = 'loss', verbose=1, save_best_only = True, mode='min')
    ...
    model.save('ptb_char_last.h5')

def text_generation(model_path, test_data, id_to_word, vocab_size, time_step = 1):
    old_model = load_model(model_path, custom_objects = {'perplexity': perplexity})
    
    model = Sequential()
    model.add(Embedding(vocab_size, 6, batch_input_shape = (1, time_step)))
    model.add(CuDNNLSTM(256, return_sequences = True, stateful = True))
    #model.add(CuDNNLSTM(256, batch_input_shape = (1, time_step, 1), stateful = True))
    model.add(Dropout(0.2))
    model.add(Dense(vocab_size, activation = "softmax"))
    ...
    f = open("ptb_char_generated.txt", 'w')
...
if __name__ == "__main__":
    ...
    #fit_model(train_data, valid_data, len(word_to_id), 1.0)
    #evaluate_model("ptb_char.h5", valid_data)
    text_generation("ptb_char_last.h5", test_data, id_to_word, len(word_to_id))
